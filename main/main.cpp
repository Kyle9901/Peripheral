...
void apply_sensor_reading(sensor_reading_t reading)
{
    if (reading.dht_valid) {
        ...
        update_attribute(s_endpoints.temperature, TemperatureMeasurement::Id,
        ...
        update_attribute(s_endpoints.humidity, RelativeHumidityMeasurement::Id,
                         RelativeHumidityMeasurement::Attributes::MeasuredValue::Id,
                         value, "湿度");
    }

    if (reading.bh1750_valid) {
        auto value = esp_matter_nullable_uint16(
            nullable<uint16_t>(lux_to_matter(reading.illuminance_lux)));
        update_attribute(s_endpoints.illuminance, IlluminanceMeasurement::Id,
                         IlluminanceMeasurement::Attributes::MeasuredValue::Id,
                         value, "光照");
    }

    auto occupancy = esp_matter_bitmap8(reading.occupied ? 0x01 : 0x00);
    update_attribute(s_endpoints.occupancy, OccupancySensing::Id,
                                    ...
                                    void *private_data)
{
    ...
    if (endpoint_id == s_endpoints.occupancy &&
...
bool create_sensor_endpoints(node_t *node)
{
    ...
    endpoint_t *occupancy = endpoint::occupancy_sensor::create(
        node, &occupancy_config, ENDPOINT_FLAG_NONE, nullptr);

    if (temperature == nullptr || humidity == nullptr || illuminance == nullptr ||
        occupancy == nullptr) {
        return false;
    }

    cluster_t *occupancy_cluster = cluster::get(occupancy, OccupancySensing::Id);
    ...
    if (attribute::get_val(hold_attribute, &persisted_hold) != ESP_OK ||
        ...
        sensor_manager_set_occupancy_hold_seconds(
            persisted_hold.val.u16) != 0) {
        ESP_LOGE(TAG, "加载 PIR 占用保持时间失败");
        return false;
    }

    s_endpoints.temperature = endpoint::get_id(temperature);
    s_endpoints.humidity = endpoint::get_id(humidity);
    s_endpoints.illuminance = endpoint::get_id(illuminance);
    s_endpoints.occupancy = endpoint::get_id(occupancy);
    ESP_LOGI(TAG,
             "Matter endpoints：温度=%u 湿度=%u 光照=%u 占用=%u，PIR保持=%u秒",
             s_endpoints.temperature, s_endpoints.humidity,
             s_endpoints.illuminance, s_endpoints.occupancy,
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "=== InstaCare Matter Multi Sensor ===");

    node::config_t node_config;
    node_t *node = node::create(&node_config, attribute_update_callback,
                                identification_callback);
    if (node == nullptr || !create_sensor_endpoints(node)) {
        ESP_LOGE(TAG, "创建 Matter 节点或传感器 Endpoint 失败");
        return;
    }
    ...
    if (sensor_manager_init(sensor_reading_callback, nullptr) != ESP_OK) {
